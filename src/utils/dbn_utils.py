...
def train_layer_wise(dbn, data_loader, lr=0.01, k=1, epochs_per_layer=10):
    ...
    for i, rbm in enumerate(dbn.rbm_layers):
        ...
        if i < len(dbn.rbm_layers) - 1:
            ...
            current_data_loader = transform_data_for_next_layer(
                rbm, current_data_loader
            )
...
def visualize_layer_training_history(layer_histories, figsize=(15, 10)):
    ...
    for layer_idx, history in layer_histories.items():
        ...
        axes[0, layer_idx].plot(
            history["epoch"], history["avg_free_energy"], "b-", marker="o"
        )
        ...
        axes[1, layer_idx].plot(
            history["epoch"], history["recon_error"], "r-", marker="o"
        )
...
def visualize_dbn_activations(dbn, input_data, layer_idx=None, cmap="viridis"):
    ...
    layers_to_show = (
        [layer_idx] if layer_idx is not None else range(len(dbn.rbm_layers))
    )
    ...
    for i in layers_to_show:
        ...
        plt.suptitle(
            f"Layer {i+1} Activations ({layer_acts.shape[1]} units)", fontsize=16
        )
        plt.tight_layout()
